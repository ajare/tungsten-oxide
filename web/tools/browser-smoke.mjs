...
// The actual repo root, not web/ -- this file is web/tools/browser-smoke.mjs (two levels up), and
// the server below must reach both web/ (editor.html, track.html, ...) and assets/ (a repo-root
// sibling of web/, see CLAUDE.md's "What this is"). Pages are requested as e.g. web/editor.html
// below, matching the paths their own import maps/BUNDLED_TEXTURE_ROOT expect relative to that.
...
await visit('editor: grid visibility toggle and shortcut disable snapping', 'web/editor.html', async (page) => {
...
await visit('editor: bundled and browsed textures save references, not embedded image data', 'web/editor.html', async (page) => {
...
await visit('editor: random generator preserves textures and builds deterministic mesh gaps, checkpoints, and boosts', 'web/editor.html', async (page) => {
  await page.setInputFiles('#textureFileInput', join(ROOT, 'assets/test-1.png'));
...
await visit('game: generated mesh section is traversable into the receiving path', 'web/track.html', async (page) => {
...
await visit('game: generated ramp sequence can return to its receiving path', 'web/track.html', async (page) => {
...
await visit('editor: both halves of a deleted segment accept textures independently', 'web/editor.html', async (page) => {
...
await visit('editor: import mesh, move, rail, export', 'web/editor.html', async (page) => {
...
await visit('editor: right-click does not read the clipboard', 'web/editor.html', async (page) => {
...
await visit('game: player remains at its starting-grid pose while parked', 'web/track.html', async (page) => {
  await page.evaluate(() => {
...
await visit('game: creates one player and seven independently simulated idle AI ships', 'web/track.html', async (page) => {
  await page.evaluate(() => {
    localStorage.setItem('web3d.currentTrack',
      window.TrackCore.serializeTrack(window.TrackCore.cloneTrack(window.TrackCore.DEFAULT_TRACK)));
  });
...
await visit('game: zones and triggers keep independent per-ship state', 'web/track.html', async (page) => {
  await page.evaluate(() => {
...
await visit('track.html loads and builds a mesh region', 'web/track.html', async (page) => {
...
await visit('game: drives on a mesh region and is stopped by rails', 'web/track.html', async (page) => {
  await page.evaluate((mesh) => {
...
await visit('game: bare edge is a ledge, and respawn recovers', 'web/track.html', async (page) => {
  await page.evaluate((mesh) => {
    const t = window.TrackCore.cloneTrack(window.TrackCore.DEFAULT_TRACK);
    t.meshAssets = { pad: { name: 'pad', railHeight: 3, mesh: JSON.parse(mesh) } };
    t.meshes = [{ id: 'm1', asset: 'pad', x: 600, z: 0, rotation: 0, elevation: 5 }];
...
await visit('game: a hole in a mesh region is not drivable', 'web/track.html', async (page) => {
  await page.evaluate((mesh) => {
    const m = JSON.parse(mesh);
    for (const e of m.edges) e.attributes = { rail: true };
    const t = window.TrackCore.cloneTrack(window.TrackCore.DEFAULT_TRACK);
...
await visit('regression: mesh-free track still drives normally', 'web/track.html', async (page) => {
  await page.evaluate(() => {
    localStorage.setItem('web3d.currentTrack',
      window.TrackCore.serializeTrack(window.TrackCore.cloneTrack(window.TrackCore.DEFAULT_TRACK)));
  });
...
await visit('game: spline guard rails reflect velocity instead of sticking the ship', 'web/track.html', async (page) => {
...
await visit('game: a ship can fly off the end of an open curve', 'web/track.html', async (page) => {
  await page.evaluate((t) => localStorage.setItem('web3d.currentTrack', JSON.stringify(t)), openTrack);
  await page.reload({ waitUntil: 'networkidle' });
