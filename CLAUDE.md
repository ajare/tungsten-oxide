...
- `npm run gen-traces` — regenerate both committed parity layers in `test/traces/`: legacy baked-world traces and current-schema raw-track traces. This is deliberate and reviewable; run only when physics, loading/baking, or the corpus intentionally changes.
- `npm run parity` — the top-level cross-check: JS↔JS replay plus C++ baked-world and raw-track replay (when `cpp/build` has been built).
...
- **Native tests.** `track_tests` covers strict loading, JS-generated normalization/geometry oracles, mesh topology and renderer attributes, and direct simulation scenarios. `parity` and `raw_parity` are separate CTest entries, preserving the original gate unchanged.
