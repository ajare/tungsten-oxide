...
The same idea applies **along** the path, not just across it: `TrackCore.buildAdaptiveMeshFrames(controlPoints, closed, rollPoints, widthPoints, crossSectionPoints, raw, edges)` takes the already-baked physics `raw`/`edges` (the fixed, uniform-in-parameter `N_DEFAULT` frames from `buildCenterline`) and produces a **separate, mesh-only** frame/edge array with denser rings on sharp turns/hills and sparser ones on long straights — pure rendering, consumed only by `js/track-game.js`'s `buildPath` for the road surface + shell. **Physics/collision never sees this.** `centerline`, `computePhysicalWallOffsets`, `sLeft`/`sRight`, and guard rails all still ride on the original fixed baking, untouched — deliberately, because physics here is genuinely frame-based (unlike cross-section height, which physics gets from a continuous analytic function), and the tuned tolerance constants documented below (`SEGMENT_ALONG_TOL`, `CORRIDOR_ALONG_TOL`) were never validated against non-uniform frame spacing.
...
- Track geometry/physics constants of note: `N_DEFAULT` centerline samples per path, `COLLISION_WALL_MARGIN` (defined in `track-core.js`). Cross-section width sampling is adaptive, not a fixed segment count — see `TrackCore.crossSectionBreakpoints`.
