...
- **Position recovery is now exact at any roll.** `projectToSurface(sample, px, py, pz)` and `sampleTrack(x, y, z)`'s membership test take the *full 3D* point and dot it against the unit `edgeRight` — no division, no degeneracy. They **require the ship's real Y**; passing the centerline's Y instead reintroduces the old singularity through the back door. Segment *selection* in `sampleTrack` stays X/Z-nearest (fine, because under pure banking the centerline itself is not vertical — only the cross-section is), so a fully vertical *centerline* climb, e.g. a straight-up loop wall, is still not handled; that's a separate, out-of-scope case.
- **Velocity lives in the tangent plane** (`vel = moveDir * speed`, a full 3D vector), so on a bank it gains the vertical component that lets the ship climb toward a rolled edge. Steering rotates `forward` about the **surface normal** (`applyAxisAngle`), which is intrinsically correct through vertical and inverted sections — this is why the old `upSign` world-yaw flip hack is gone. `tangentize()` re-flattens `forward`/`moveDir` onto the surface every frame as the normal changes underneath them.
