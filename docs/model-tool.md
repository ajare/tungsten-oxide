# src/model-tool — 3D model import/preview/save utility (`model_tool`)

`src/model-tool` is a standalone native app (Windows/MSVC) that imports a 3D model file via AssImp, previews it in a live `mpp::RenderSystem` viewport, and saves it as `.mppmodel` (MassivePolyPusher's binary model format, documented in `MPPMODEL_EXPORT_SPEC.md`).

**Status note:** the design record, `docs/adr/0001-model-tool.md`, still carries `Status: Accepted (design only — not yet implemented)`. That is stale — the tool is fully built and shipping (wired into the combined `src/` build via `add_subdirectory(model-tool)` in `src/CMakeLists.txt`), with several of the ADR's design decisions since superseded by what actually shipped (noted below). Treat this document, not the ADR header, as current.

## Intended use

A standalone offline utility for getting a third-party 3D model into `.mppmodel` form, and (since `TRACK_MODEL_LIST_PLAN.md`, `docs/adr/0003-model-xml-layer.md`) for authoring the `<Model>` XML fragment that carries each mesh's Type/Visible metadata alongside it. It shares `src/model-xml` (a small TinyXML2-based library, see that plan's Milestone 2) with `src/editor` — the one place the two apps share code — but nothing else at runtime beyond the `.mppmodel` file format itself; NormalSmoothing.cpp/ObjSmoothingGroups.cpp are still compiled only here. It is not wired into the editor's own UI or the game runtime beyond that shared fragment-schema library; a `.mppmodel` this tool saves is consumed the same way any other `.mppmodel` is, by `src/tungsten-monoxide/src/Map.cpp` (see `DRIVABLE_MESH_OBJECTS_PLAN.md` Milestone 3's architecture note and Milestone 4 below) or by `src/editor`'s own placement-rendering path (`TRACK_MODEL_LIST_PLAN.md` Milestone 4).
...
- **Fixed vertex layout**: `struct ImportedVertex` — position (float×3), normal (float×3), uv (float×2), color (unorm8×4) = **36 bytes**, matching the stride `src/editor`'s own `MppModelExport.cpp` uses for track geometry (independent implementations, same shape by convention). Missing color defaults to opaque white, missing UV to (0,0).
...
`include/MppSave.hpp`/`src/MppSave.cpp` — `saveModelAsMppModel()` uses the real `mpp::ModelSerializer` (unlike `src/editor`'s hand-written exporter — see `docs/editor.md`), preserving shared-vertex indexing rather than an unshared triangle soup.
...
- **Open...** (`Ctrl+O`) — a `<Model>` XML fragment only: a standalone file (bare `<Model>` root) or one embedded in a Track resource XML's `<Models>` list (prompting a "Choose Model" picker if it embeds more than one, mirroring `src/editor`'s own). Reads the referenced `.mppmodel` and applies that fragment's per-mesh Type/Visible metadata onto the freshly loaded meshes.
...
Single `main()` (`src/model-tool/main.cpp`). SDL2 window, fixed DockBuilder layout (Panels left, Viewport right, no saved layout — identical every launch), a main menu bar, and a timed status bar.
