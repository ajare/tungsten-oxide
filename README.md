...
In the editor, **Import Mesh** loads a `.json` file and drops it into the middle of the current view. **Paste Mesh** reads the clipboard instead — pair it with **Copy JSON** in the geometry-js editor — and places the region at the world origin, so its authored coordinates are preserved. In Edit mode you can also right-click the top-down view: when the clipboard holds something, the menu grows an **Add mesh → From clipboard** entry that drops the region centred on where you clicked. Click to select it, drag to move, and set its X/Z, elevation and rotation in the properties panel (or drag its line in the elevation panel to match a ribbon's height). Switch the mode dropdown to **Rails** (or press `R`) and click individual edges to toggle them:
