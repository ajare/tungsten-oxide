...
// reimplementation of the same documented <Model> fragment shape src/model-xml parses via TinyXML2
// for the editor/model-tool, per that plan's own architecture note on why this host doesn't link
// src/model-xml. Exactly one <Model> must carry a Type=Track mesh (and therefore <TrackData>) --
