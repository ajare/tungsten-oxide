...
//
// Unlike src/editor's retired from-scratch byte writer, this drives the real serializer, so the
...
// before touching indices -- src/tungsten-monoxide's Map.cpp, and readMppModel() below -- handle
// that correctly. mpp::MppModelStream does not: it calls getIndexData()/getIndexWidth()
// unconditionally. This matches the convention src/editor's shipped track exports already use, so
...
// material by name only, to be resolved by the consuming project. This is what src/model-tool and
// src/editor's track export both want: they have no PBR pipeline to validate against, and their
