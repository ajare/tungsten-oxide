// src/editor/main.cpp — track_editor: native ImGui/SDL3/OpenGL track editor.
...
// Locates src/editor/resources/<filename> by walking up from the current working directory --
// mirrors TextureCache.hpp's findAssetsDir() for the same reason: the editor's build output
// directory (e.g. build/editor/Release) is nested several levels under the repo root, so a
...
// intentionally walks up from the CWD to source-tree src/editor/resources/, since the
...
int main(int, char**) {
  ...
  // icon-font recipe -- see src/editor/include/fontawesome/README-VENDORED.md). Missing the font
  ...
  while (running) {
    ...
    if (ImGui::BeginMainMenuBar()) {
      if (ImGui::BeginMenu("File")) {
        ...
        // latter parsed via src/model-xml, same as model-tool's own OpenTarget.cpp classification --
        ...
        // PbrMaterial .mppmodel via src/model-io, same as gltf_convert, but through a modal that
