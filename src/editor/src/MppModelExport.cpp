...
MppModelExportResult exportTrackToMppModel(const tox::Track& track, const std::map<std::string, std::string>& trackMaterialToMaterial) {
  ...
  // index buffer holding 0,1,2,3,... would be pure redundancy; src/tungsten-monoxide's Map.cpp
...
std::string buildTrackResourceXmlForName(const TrackDefinition& track, const tox::Track& bakedTrack,
...

  {
    ...
    // id == ref (the qualified name) -- Map::load() (src/tungsten-monoxide/src/Map.cpp) resolves
  ...
  // src/tungsten-monoxide/src/DLL.cpp). Omitting it throws "could not find a definition factory".
