...
// src/model-io's modelio::writeMppModelWithNamedMaterials, itself backed by the real
...
// non-indexed. `src/tungsten-monoxide`'s Map.cpp accepts both strides (docs/GLTF_IMPORT_PLAN.md
...
// src/tungsten-monoxide/resources/Resources.yaml) declaring this track as a `type="Track"`
...
// src/core/src/TrackBake.cpp's and TrackMesh.cpp's hardcoded materialKey strings). Meant to be
