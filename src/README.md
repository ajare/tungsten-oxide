...
`src/core` is a C++20 static library that independently loads and runs complete
...
cmake --build build --config MemCheck
ctest --test-dir build -C MemCheck --output-on-failure
```
...
`src/core` can also be configured directly; it imports the prebuilt `ext/willpower/build`
...
The fixture/trace corpus under `src/test-data/` is a fixed, committed regression suite; there is no
...
`src/test-data/fixtures/random-track-mesh/README.md`.

## Native runtime host

`src/app` builds `track_runner`, a minimal command-line host outside `core`
(only added by the combined root configure, not standalone `src/core`):
