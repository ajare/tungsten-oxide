// src/model-tool/main.cpp — model_tool: imports OBJ/FBX/USD/glTF via AssImp, or an existing
...
// Locates src/model-tool/resources/<filename> by walking up from the current working directory --
// mirrors src/editor/main.cpp's own findEditorResourceFile() for the same reason: model_tool's
...
// Keys every viewport resource against, mirroring src/tungsten-monoxide's TmResourceWrangler
...
int main(int argc, char** argv) {
  ...
  // codebase (src/launcher, ext/willpower/ext/massive-poly-pusher/demo-suite).
  ...
  // matching src/editor's own docked-shell convention) is meant to be the same every launch.
  ...
  // mirrors src/editor/main.cpp's identical setup, see src/editor/include/fontawesome/
  ...
  // AddFontDefault() needs an explicit SizePixels here -- see src/editor/main.cpp's identical
  ...
  // Status bar message, mirrors src/editor/main.cpp's own showStatus() convention: the most recent
  ...
  while (running) {
    ...
    // steal these keys (mirrors src/editor/main.cpp's own !io.WantTextInput-guarded shortcut block).
    ...
    // G toggles the viewport's reference grid, unmodified (mirrors src/editor's own G shortcut for
    ...
    if (ImGui::BeginMainMenuBar()) {
      ...
      if (ImGui::BeginMenu("View")) {
        // Mirrors src/editor's own "Show Grid" / "Grid Size" View menu entries (main.cpp) and
    ...
    // to conflict with) -- left panel + right viewport, mirroring src/editor's own left-panel-plus-
    // views arrangement.
  ...
  // ext/willpower/ext/massive-poly-pusher/demo-suite's own shutdown() ordering.
  delete viewport;
