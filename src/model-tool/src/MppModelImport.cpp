...
// written with NO index stream -- a real, legitimate shape: src/editor's own Track exports
...
std::optional<MppModelImportResult> importMppModel(const std::string& utf8Path, mpp::ResourceManager& resourceMgr,
  ...
  try {
    ...
    for (std::size_t i = 0; i < meshCount; ++i) {
      ...
      // A mesh written with NO index stream (a real, legitimate shape: src/editor's own Track
