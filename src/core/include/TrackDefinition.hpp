...
struct PathDefinition {
  ...
  // src/editor's Materials panel; empty when absent (older tracks never set this).
...
// list (see the plan's "`.mppmodel` loading is host-only"/"Models never enter `src/core`" architecture
