...
// collects gameplay events the golden-trace corpus's replayer (src/core/tests/parity_main.cpp) never
...
// (src/core/tests/track_tests.cpp's hull-vs-wall scenarios) and doesn't need to be reproduced here
// too.
...
// Mirrors src/core/tests/parity_main.cpp's loadShip() field-for-field, in reverse -- see that
...
json dumpShip(const Ship& ship) {
  ...
  return json{{"physics", physics},          {"prevTriggerPos", dumpVec(ship.prevTriggerPos)}, {"zoneInside", zoneInside},
              ...
              // src/core/tests/parity_main.cpp's loadShip() comment on why a mesh-mode trace needs
...
int runCapture(const std::filesystem::path& outputPath, const std::string& traceName, const std::filesystem::path& trackPath,
  ...
  for (int frame = 0; frame < steps; ++frame) {
    ...
    // contract; src/core/tests/parity_main.cpp's replay needs the identical fix to stay consistent
