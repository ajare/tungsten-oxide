// ElevationView.hpp — the elevation profile side view, a second ImDrawList canvas alongside the
// top-down view: x is position along the current path, y is world elevation.
//
// The x-axis is true cumulative XZ arc length along the baked centerline -- each control
...
// way, so hiding position handles in one view hides the ability to add them in both. Returns true
// if the track was mutated this frame
