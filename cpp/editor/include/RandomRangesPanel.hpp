...
// `RandomTrack.hpp`'s generateRandomTrack already accepted a RandomTrackRanges parameter (M7a/M7c);
// this is the UI to edit one, which main.cpp previously never exposed (always passing the `{}`
// default).
//
// Unlike EditorState's own fields, these ranges are a generator *preference*, not authored track
// data -- they don't round-trip through the track JSON, aren't undoable, and reset to
// RANDOM_RANGE_DEFAULTS every session, since this editor has no established
// local-settings-persistence mechanism yet (see TopDownView's grid/snap prefs for the same
// session-only tradeoff).
#pragma once
