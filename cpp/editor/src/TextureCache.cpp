...
const LoadedTexture& TextureCache::get(const std::string& path) {
  ...
  // Bundled texture assets are stored as "assets/track/..." (portable, repo-root-relative), which
  // only resolves against the current working directory when track_editor.exe happens to be
  // launched from the repo root. findAssetsDir() already walks up from the CWD to locate the
  // checked-in assets/ directory for reading manifest.json; reuse its parent (the repo root) as the
  // anchor here too, so a relative asset path resolves the same way regardless of where
  // track_editor.exe was launched from (e.g. cpp/build/editor/Release). Browse...-picked textures
  // are stored as absolute paths and pass through untouched.
  std::filesystem::path resolved(path);
  if (resolved.is_relative()) {
    const std::filesystem::path assetsDir = findAssetsDir();
    if (!assetsDir.empty()) resolved = (assetsDir.parent_path() / resolved).lexically_normal();
  }
