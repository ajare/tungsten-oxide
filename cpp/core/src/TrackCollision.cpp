...
bool segmentBounds(const Vec3& from, const Vec3& to, const TrackCollisionSurface::Bounds& bounds) {
  const Vec3 d = to.clone().sub(from);
...
std::optional<CollisionHit> intersect(const CollisionTriangle& triangle, int triangleIndex,
  ...
  const Vec3 d = to.clone().sub(from);
  const Vec3 e1 = triangle.positions[1].clone().sub(triangle.positions[0]);
  const Vec3 e2 = triangle.positions[2].clone().sub(triangle.positions[0]);
  Vec3 p;
  p.crossVectors(d, e2);
  const double det = e1.dot(p);
  if (std::fabs(det) < EPSILON) return std::nullopt;
  const double invDet = 1.0 / det;
  const Vec3 s = from.clone().sub(triangle.positions[0]);
  const double u = s.dot(p) * invDet;
  if (u < -EPSILON || u > 1.0 + EPSILON) return std::nullopt;
  Vec3 q;
  q.crossVectors(s, e1);
  const double v = d.dot(q) * invDet;
  ...
  Vec3 normal = triangle.normals[0].clone().multiplyScalar(w).addScaledVector(triangle.normals[1], u).addScaledVector(triangle.normals[2], v);
  if (normal.lengthSq() < EPSILON) return std::nullopt;
  normal.normalize();
  ...
  if (swept && d.dot(normal) >= -EPSILON) return std::nullopt;

  return CollisionHit{from.clone().addScaledVector(d, t), normal, t, triangleIndex,
                      triangle.surfaceId};
}
}  // namespace
...
int TrackCollisionSurface::build(std::size_t begin, std::size_t end) {
  ...
  for (std::size_t i = begin; i < end; ++i) {
    ...
    centroid.multiplyScalar(1.0 / 3.0);
  ...
  const Vec3 extent = centroids.hi.clone().sub(centroids.lo);
  ...
  std::nth_element(order_.begin() + static_cast<std::ptrdiff_t>(begin),
                   ...
                   order_.begin() + static_cast<std::ptrdiff_t>(end), [&](std::size_t a, std::size_t b) {
                     Vec3 ca, cb;
                     for (const Vec3& p : triangles_[a].positions) ca.add(p);
                     for (const Vec3& p : triangles_[b].positions) cb.add(p);
...
void TrackCollisionSurface::queryAxisSegment(int nodeIndex, const Vec3& from, const Vec3& to,
  ...
  for (std::size_t i = node.begin; i < node.begin + node.count; ++i) {
    ...
    if (!hit || hit->normal.dot(up) <= EPSILON) continue;
    if (!best || hit->position.distanceToSquared(origin) < best->position.distanceToSquared(origin))
      best = std::move(hit);
  }
}
...
std::optional<CollisionHit> TrackCollisionSurface::nearestAlongAxis(const Vec3& origin,
  ...
  if (nodes_.empty() || axis.lengthSq() < EPSILON || maxDistance <= 0) return std::nullopt;
  const Vec3 up = axis.clone().normalize();
  const Vec3 from = origin.clone().addScaledVector(up, maxDistance);
  const Vec3 to = origin.clone().addScaledVector(up, -maxDistance);
