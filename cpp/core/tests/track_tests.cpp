...
int main(int argc, char** argv) {
  ...
  {
    // Reservation wall UVs: U runs against the wall's run direction, V spans its height normalized
    // to [0,1] rim-to-top, with U measured in units of that same height so tiles are square in
    // world metres. (The underlying parameterization is the road surface's across-first one, turned
    // three-quarters of a turn by `wallUv` -- the quarter turn plus a further 180.)
    json input = base;
    input["version"] = 11;
    for (int i : {8, 9}) {
      input["paths"][0]["points"][i]["curvature"] = -0.5;
    ...
    if (loaded) {
      ...
      if (wall != loaded.track->geometry.end()) {
        ...
        // V is the one pinned to [0,1] and U is the one that advances. The extra half turn on top of
        // the quarter turn reverses U's direction -- U starts near 1 at the wall's own origin and
        // *decreases* without bound as the run advances, rather than increasing from 0 -- so unlike
        // before, U going negative is expected, not a defect.
        double minRun = 1e30;
        bool heightInRange = true;
        for (const auto& v : wall->vertices) {
          ...
          if (v.uv.y < -1e-9 || v.uv.y > 1.0 + 1e-9 || (std::fabs(v.uv.y) > 1e-9 && std::fabs(v.uv.y - 1.0) > 1e-9))
            heightInRange = false;
          minRun = std::min(minRun, v.uv.x);
        }
        check(heightInRange, "every wall UV's V spans the height and is exactly 0 (rim) or 1 (top)");
        check(minRun < -1.0, "U actually advances (in reverse) along the wall -- min " + std::to_string(minRun));
        ...
        for (std::size_t i = 0; i + 2 < wall->indices.size(); i += 3) {
          for (int k = 0; k < 3; ++k) {
            ...
            if (dv > 0.5 && du < 1e-9) {  // spans the height at one station
              ...
              // V is 0 at the rim and 1 at the top, the same way round on every quad. This fixture
        ...
        check(vOrientationConsistent, "V is 0 at the rim and 1 at the top on every wall quad -- a rotation, not a mirror");
