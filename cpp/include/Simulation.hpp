// Simulation.hpp — the physics step, transliterated line-for-line from
// js/track-physics.js (Simulation.stepPhysics + its helpers).
//
...
struct Projection { Vec3 er; double s{0.0}, loS{0.0}, hiS{0.0}; };
struct SurfaceFrame { Vec3 pos, normal; };
struct StepResult { Vec3 surfaceNormal, surfaceRenderPos; bool respawned{false}; };

// --- pure helpers ----------------------------------------------------------
inline double effectiveMaxSpeed(const Physics& p) {
...
inline void triggerBoost(Ship& ship, const Zone& zone) {
...
inline void tickBoost(Ship& ship, double dt) {
  Physics& p = ship.physics;
  if (!p.boostActive) return;
  if (!p.boostReleasing) {
    p.boostHold -= dt;
    p.boostEffCap = p.boostCap;
    if (p.boostHold <= 0) { p.boostReleasing = true; p.boostReleaseT = Consts::ZONE_RELEASE; }
  } else {
    p.boostReleaseT -= dt;
    ...
    if (p.boostReleaseT <= 0) { p.boostActive = false; p.boostEffCap = p.maxSpeed; }
  }
}

inline Projection projectToSurface(const Sample& s, double px, double py, double pz) {
  Projection out;
  out.er = s.edgeRight;
  ...
  if (loS > hiS) { const double m = (loS + hiS) / 2.0; loS = m; hiS = m; }
  out.loS = loS; out.hiS = hiS;
  return out;
}

inline bool corridorContains(const Sample& s, double x, double y, double z, const Projection& proj) {
...
inline SurfaceFrame curvedSurfaceFrame(const Sample& s, double sOff) {
  ...
  Vec3 normal; normal.crossVectors(s.tangent, crossT).normalize();
  if (normal.dot(s.normal) < 0) normal.negate();
  return { pos, normal };
}

// Re-project unit-ish v into the plane tangent to n; fall back when parallel.
inline Vec3& tangentize(Vec3& v, const Vec3& n, const Vec3& fallback) {
  const double d = v.dot(n);
  Vec3 tmp = v.clone();
  tmp.addScaledVector(n, -d);
  if (tmp.lengthSq() < 1e-9) { v.copy(fallback); return v; }
  v.copy(tmp).normalize();
  return v;
}

inline double signedAngleAbout(const Vec3& a, const Vec3& b, const Vec3& axis) {
  const double d = TrackCore::clamp(a.dot(b), -1.0, 1.0);
  const double ang = std::acos(d);
  Vec3 cross; cross.crossVectors(a, b);
  return cross.dot(axis) < 0 ? -ang : ang;
}

inline void beginAirborne(Ship& ship, const Vec3& vel3D) {
  Physics& p = ship.physics;
  p.airborne = true;
  p.verticalVel = vel3D.y;
  const double horiz = std::hypot(vel3D.x, vel3D.z);
  p.speed = horiz;
  if (horiz > 1e-6) p.moveDir.set(vel3D.x / horiz, 0, vel3D.z / horiz);
  else tangentize(p.moveDir, UP, p.forward);
}

inline void landOnSurface(Ship& ship, const Vec3& normal) {
  Physics& p = ship.physics;
  p.airborne = false;
  p.verticalVel = 0;
...
inline double weightRestitution(const Physics& p) {
...
inline double weightSpeedRetain(const Physics& p) {
  const double m = (p.weight != 0.0 ? p.weight : Consts::HANDLING_BASE_WEIGHT) / Consts::HANDLING_BASE_WEIGHT;
  return std::max(0.85, std::min(0.999, 1.0 - 0.02 / m));
}
inline void addImpactJolt(Physics& p, double normalImpactSpeed) {
  const double m = (p.weight != 0.0 ? p.weight : Consts::HANDLING_BASE_WEIGHT) / Consts::HANDLING_BASE_WEIGHT;
...
class Simulation {
public:
  explicit Simulation(const Track& track) : track_(track) {}

  Sample sampleTrack(double x, double y, double z) const {
    const auto& paths = track_.paths;
    struct Cand { int path{0}, a{0}, b{1}; double t{0.0}, d{1e300}; bool valid{false}; };
    Cand fallback; fallback.path = 0; fallback.d = 1e300;
    Cand bestUnder;
    ...
    for (int pi = 0; pi < (int)paths.size(); ++pi) {
      ...
      for (int i = 0; i < segCount; ++i) {
        ...
        double erl = std::hypot(erx, ery, erz); if (erl == 0.0) erl = 1.0;
    ...
    Sample smp;
    smp.pathIndex = best.path; smp.a = best.a; smp.b = best.b; smp.segT = t;
  ...
  // The ship's evaluator parameter g on the path the sample landed on, recovered
  // from the (a,b,segT) sampleTrack recorded (mirror of track-physics.js shipParamG).
  ...
  // Enter/leave detection for path boost zones (mirror of track-physics.js
  // detectZoneTriggers; mesh zones are out of scope so `meshRegion` is false).
  ...
  // Swept crossing of the ship segment p0->p1 against every trigger gate (mirror
  // of track-physics.js detectTriggers).
  ...
  void fireTrigger(Ship& ship, const Trigger& rec, const std::string& dir) const {
    ...
    Race& race = ship.race;
    if (rec.role != "finish") { race.hit.insert(rec.id); return; }
    for (const std::string& id : race.intermediateIds) if (!race.hit.count(id)) return;
    race.laps++;
    race.hit.clear();
  ...
  void clearBoost(Ship& ship) const {
    Physics& p = ship.physics;
    p.boostActive = false; p.boostReleasing = false;
    p.boostHold = 0; p.boostReleaseT = 0; p.boostCap = 0; p.boostEffCap = 0;
  ...
  void placeShipAtPose(Ship& ship, const Pose& pose, const std::string& disarmedId) const {
    ...
    p.up.copy(pose.up); p.visualUp.copy(pose.up);
    p.right.crossVectors(pose.up, pose.forward).normalize();
    p.heading = std::atan2(pose.forward.x, pose.forward.z);
    p.speed = 0; p.airborne = false; p.verticalVel = 0;
    p.visualBank = 0; p.visualPitch = 0;
    p.landingBounce = 0; p.landingBounceVel = 0;
  ...
  void respawn(Ship& ship) const {
    ...
    if (cp.valid) placeShipAtPose(ship, Pose{ cp.pos, cp.up, cp.forward }, cp.triggerId);
    else placeShipAtPose(ship, ship.startPose, std::string());   // no id disarmed
  }
  ...
  StepResult stepPhysics(Ship& ship, double dt, double throttle, double brake, double steer) const {
    ...
    if (throttle) {
      ...
      if (p.speed > 0) p.speed = std::max(0.0, p.speed - decay);
      else p.speed = std::min(0.0, p.speed + decay);
    }
