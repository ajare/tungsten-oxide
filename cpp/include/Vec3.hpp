...
//
// Do NOT "optimize" the method bodies:
...
//     ULP because float add isn't associative.
...
struct Vec3 {
  ...
  Vec3& set(double x_, double y_, double z_) { x = x_; y = y_; z = z_; return *this; }
  Vec3 clone() const { return Vec3(x, y, z); }
  Vec3& copy(const Vec3& v) { x = v.x; y = v.y; z = v.z; return *this; }

  Vec3& add(const Vec3& v) { x += v.x; y += v.y; z += v.z; return *this; }
  Vec3& addVectors(const Vec3& a, const Vec3& b) { x = a.x + b.x; y = a.y + b.y; z = a.z + b.z; return *this; }
  Vec3& addScaledVector(const Vec3& v, double s) { x += v.x * s; y += v.y * s; z += v.z * s; return *this; }
  Vec3& sub(const Vec3& v) { x -= v.x; y -= v.y; z -= v.z; return *this; }
  Vec3& subVectors(const Vec3& a, const Vec3& b) { x = a.x - b.x; y = a.y - b.y; z = a.z - b.z; return *this; }
  Vec3& multiplyScalar(double s) { x *= s; y *= s; z *= s; return *this; }
  Vec3& divideScalar(double s) { return multiplyScalar(1.0 / s); }
  ...
  double distanceTo(const Vec3& v) const { return std::sqrt(distanceToSquared(v)); }
