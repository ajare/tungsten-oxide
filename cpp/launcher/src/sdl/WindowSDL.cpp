...
SDL_GLContext WindowSDL::getContext() const {
  return mContextGL;
}
...
void WindowSDL::create() {
  ...
  if (!SDL_GL_SetSwapInterval(mVSync ? 1 : 0)) {
    gLogger->info("Could not set the requested SDL swap interval.");
  }
}
...
application::KeyModifiers WindowSDL::getKeyModifiers(SDL_Keymod mod) {
    ...
    switch (evt.type) {
      ...
      case SDL_EVENT_MOUSE_BUTTON_DOWN:
        if (evt.button.button <= SDL_BUTTON_MIDDLE) {
          stateMgr->injectMouseButtonInput(application::MouseButtonEvent::Pressed,
                                           mButtonTranslator[evt.button.button], getKeyModifiers(SDL_GetModState()));
        }
        break;

      case SDL_EVENT_MOUSE_BUTTON_UP:
        if (evt.button.button <= SDL_BUTTON_MIDDLE) {
          stateMgr->injectMouseButtonInput(application::MouseButtonEvent::Released,
                                           mButtonTranslator[evt.button.button], getKeyModifiers(SDL_GetModState()));
        }
        break;

      case SDL_EVENT_MOUSE_WHEEL:
        stateMgr->injectMouseWheelInput(static_cast<int>(evt.wheel.y));
        break;
